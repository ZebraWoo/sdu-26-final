...
class PCRLoss(nn.Module):
    ...
    def forward(self, student_feats, teacher_feats, pcr_bin_matrix):
        ...
        teacher_feats = F.normalize(teacher_feats, dim=-1)  # [2*B, N, D64]

        B = teacher_feats.shape[0] // 2
        teacher_feats_reorder = torch.cat([teacher_feats[B:], teacher_feats[:B]]) # [img0_crop0, img1_crop0,....img64_crop0, img0_crop1,img1_crop1,...img64_crop1]
        bi_bin_matrix = torch.cat([pcr_bin_matrix, pcr_bin_matrix.transpose(-1,-2)], dim=0)  # [2*B, N, N]
        pos_mask = bi_bin_matrix.bool()
        sim_matrix_s2t = torch.matmul(student_feats, teacher_feats_reorder.transpose(-1, -2))
        # 1. scale logits
        logits = sim_matrix_s2t / 0.1   # [2B, N, N]
