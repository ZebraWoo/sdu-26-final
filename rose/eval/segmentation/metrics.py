...
logger = logging.getLogger("rose_segmentation_metrics")


SEGMENTATION_METRICS = ["mIoU", "acc", "aAcc", "dice", "fscore", "precision", "recall"]
SEGMENTATION_METRICS_PATH = "/data/wenjing/skin_dataset/panderm_eval/segmentation/ham10k_panderm_linear_seg_output"
...
def calculate_segmentation_metrics(
    ...
    if mode == "test":
        compute_per_sample_metrics_and_save(pre_eval_results, SEGMENTATION_METRICS_PATH)
...
def calculate_intersect_and_union(pred_label, label, num_classes, ignore_index=999, reduce_zero_label=False):
...
def compute_per_sample_metrics_and_save(
    pre_eval_results,
    save_path,
    eps=1e-7,
):
    ...
    Args:
        pre_eval_results:
            tuple of (area_intersect, area_union, area_pred_label, area_label)
            each is a tuple of length N, element shape = [2,]
        save_path: str, e.g. "per_sample_metrics.npy"
    """

    area_intersect, area_union, area_pred_label, area_label = pre_eval_results

    num_samples = len(area_intersect)
    ...
    for i in range(num_samples):
        # foreground only (index = 1)
        I = area_intersect[i][1].item()
        U = area_union[i][1].item()
        P = area_pred_label[i][1].item()
        G = area_label[i][1].item()
    ...
    save_dict = {
        "DSC": np.array(dsc_list, dtype=np.float32),
        "JAC": np.array(jac_list, dtype=np.float32),
        "precision": np.array(precision_list, dtype=np.float32),
        "recall": np.array(recall_list, dtype=np.float32),
    }
    save_path = os.path.join(save_path, "per_sample_segmentation_metrics.npy")
    np.save(save_path, save_dict)
    logger.info(f"Saved per-sample segmentation metrics to {save_path}")
