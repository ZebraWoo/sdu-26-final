...
from rose.configs import DinoV3SetupArgs, setup_config
...
def load_model_and_context(model_config: ModelConfig, output_dir: str) -> tuple[torch.nn.Module, BaseModelContext]:
    if model_config.model_name == "ROSE":
    ...
    elif model_config.model_name == "PanDerm":
        model = panderm.panderm_large_patch16_224(pretrained=True)
        state_dict = torch.load(model_config.pretrained_weights, map_location='cpu', weights_only=True)
        state_dict = {k.replace("encoder.", ""): v for k, v in state_dict.items()}
        model.load_state_dict(state_dict, strict=False)
        base_model_context = BaseModelContext(autocast_dtype=torch.bfloat16)
    elif model_config.model_name == "SwAVDerm":
        model = TimmCNNEncoder(weights_path="/data/wenjing/skin_dataset/swavderm_eval/model.safetensors",kwargs = {'features_only': True, 'out_indices': (4,), 'pretrained': False, 'num_classes': 0})
        checkpoint = torch.load(model_config.pretrained_weights, map_location='cpu', weights_only=False)
