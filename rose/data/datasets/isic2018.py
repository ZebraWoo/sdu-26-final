...
def _file_to_segmentation_path(file_name: str, segm_base_path: str) -> str:
    return os.path.join(segm_base_path, file_name + "_segmentation.png")
...
def _load_file_paths(root: str, split: _Split) -> Tuple[List[str], List[str]]:
    with open(os.path.join(root, f"isic2018{split.value}.csv")) as f:
        df = pd.read_csv(f)
        split_file_names = df["isic_id"].tolist()

    all_segmentation_paths = _load_segmentation(root, split_file_names)
    file_names = [os.path.join("images", el + ".jpg") for el in split_file_names]
    return file_names, all_segmentation_paths


class isic2018(ExtendedVisionDataset):
    Split = Union[_Split]
    Labels = Union[Image.Image]

    def __init__(
        self,
        split: "isic2018.Split",
