...
class Dermoscopy(ExtendedVisionDataset):
    Split = Union[_Split]

    def __init__(
        self,
        split: "Dermoscopy.Split",
        root: str,        
        extra: str,
        meta: str = "merged_ssl.csv",
    ...
    def _dump_entries(self) -> np.ndarray:
        split = self._split
        if split == Dermoscopy.Split.TRAIN:
            meta_path = self.meta
            ...
            meta_df = pd.read_csv(meta_full_path, dtype=str)
    ...
    def get_image_data(self, index: int) -> bytes:
        ...
        base = os.path.join(self.ssl_data, entry["source_dataset"], entry["image_id"])

        jpg_path = base + ".jpg"
        JPG_path = base + ".JPG"

        if os.path.isfile(jpg_path):
            image_full_path = jpg_path
        elif os.path.isfile(JPG_path):
            image_full_path = JPG_path
        else:
            raise FileNotFoundError(
                f"Image not found: {jpg_path} or {JPG_path}"
            )
