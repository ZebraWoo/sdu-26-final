...
from rose.data.transforms import SSL_DEFAULT_MEAN, SSL_DEFAULT_STD, make_normalize_transform
from rose.eval.segmentation.metrics import preprocess_nonzero_labels
...
class RandomCropWithLabel(torch.nn.Module):
    ...
    def __init__(self, crop_size, cat_max_ratio=0.75, ignore_index=999):
        super().__init__()
...
class MaskToTensor(torch.nn.Module):
    ...
    def forward(self, img, label):
        ...
        if torch.unique(Mask(label)).tolist() == [0, 255]:
            label = label.point(lambda p: 1 if p == 255 else p)
...
def make_segmentation_train_transforms(
    *,
    ...
    mean: Sequence[float] = [mean * 255 for mean in SSL_DEFAULT_MEAN],
    std: Sequence[float] = [std * 255 for std in SSL_DEFAULT_STD],
):
    ...
    if crop_size:
        transforms_list.append(PadTensor(pad_shape=crop_size, img_pad_value=0, label_pad_value=0))
