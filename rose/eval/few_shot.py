...
from rose.eval.data import create_train_dataset_dict, get_num_classes, pad_multilabel_and_collate
...
from rose.eval.utils import save_results as default_save_results_func
...
class TrainConfig:
    ...
    avg: bool = True # whether to use avgpool along with class token in linear classifier input
    ...
    checkpoint_retention_policy: CheckpointRetentionPolicy = CheckpointRetentionPolicy.BEST  # keep checkpoints or not
...
@dataclass
class FewShotConfig:
    enable: bool = True  # whether to use few-shot evaluation
    k_or_percent:  Optional[float] = 0.5   # number of elements or % to take per class
    n_tries: int = 3  # number of tries for few-shot evaluation
...
class LinearEvalConfig:
    ...
    save_results: bool = True  # save predictions and targets in the output directory
    output_dir: str = ""
...
def setup_linear_classifiers(sample_output, n_last_blocks_list, learning_rates, batch_size, avg, num_classes=1000):
    ...
    if distributed.is_enabled():
        linear_classifiers = nn.parallel.DistributedDataParallel(linear_classifiers, device_ids=[torch.cuda.current_device()])
...
def make_eval_data_loader(
    *,
    ...
    class_mapping = None
    if hasattr(test_dataset, "get_class_mapping"):
...
class Evaluator:
    ...
    def __post_init__(self):
        ...
        self.best_accuracy_seen = -1  # Track best accuracy for this evaluator
    ...
    def evaluate_and_maybe_save(
        ...
        current_accuracy = full_results_dict["best_classifier"]["accuracy"]
        # Only save results if this is the best accuracy we've seen
        if self.save_results_func is not None and current_accuracy > self.best_accuracy_seen:
            logger.info(f"current_best_acc:{self.best_accuracy_seen}")
            self.best_accuracy_seen = current_accuracy
            logger.info(f"Saving predictions with best model, current_acc:{current_accuracy}")
            self.save_results_func(
                filename_suffix = f"{self.dataset_str.replace(':', '=').split('=')[0]}_{self.dataset_str.replace(':', '=').split('=')[2]}{save_filename_suffix}",
                **accumulated_best_results
                )

        results_dict = {
            self.main_metric_name: 100.0 * current_accuracy,
    ...
    for data, labels in metric_logger.log_every(
        ...
        if is_ckpt_iteration:
            ...
            if distributed.is_subgroup_main_process():
                ...
                logger.info(f"save {iteration} its checkpoint")
...
def eval_linear_with_model(*, model: torch.nn.Module, autocast_dtype, config: LinearEvalConfig):
    ...
    save_results_func = None  #  modify 1229
    if config.save_results:

        # save_results_func = partial(default_save_results_func, output_dir=config.output_dir)
        # 获取 few-shot 比例并转为整数百分比（0.1 → 10, 1.0 → 100）
        if hasattr(config, 'few_shot') and getattr(config.few_shot, 'enable', False):
            k_or_p = config.few_shot.k_or_percent
            if isinstance(k_or_p, float):
                fs_pct = int(k_or_p * 100)  # 0.1 → 10
            else:
                fs_pct = int(k_or_p)        # 如果是整数 k，直接用（可选处理）
        else:
            fs_pct = 100  # full-shot 默认 100

        # 自定义保存函数，注入 fs_pct
        def custom_save_results_func(filename_suffix: str, **kwargs):
            # 插入 fs_pct 到文件名中
            parts = filename_suffix.split('_', 2)  # ['MNDICHD', 'TEST', '0'] 或 ['MNDICHD', 'TEST']
            if len(parts) >= 2:
                dataset_name = parts[0]
                split_part = parts[1]
                try_id_part = parts[2] if len(parts) > 2 else ""
                new_suffix = f"{dataset_name}_{fs_pct}_{split_part}"
                if try_id_part:
                    new_suffix += f"_{try_id_part}"
            else:
                new_suffix = f"{fs_pct}_{filename_suffix}"
            default_save_results_func(
                output_dir=config.output_dir,
                filename_suffix=new_suffix,
                **kwargs
            )

        save_results_func = custom_save_results_func
    ...
    for checkpoint_output_dir in checkpoint_output_dirs:
        if distributed.is_subgroup_main_process():
            logger.info("clean pth")
            cleanup_checkpoint(checkpoint_output_dir, CheckpointRetentionPolicy.BEST)
