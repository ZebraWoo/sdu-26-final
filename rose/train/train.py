...
from rose.configs import setup_config, setup_job, setup_multidistillation
from rose.data import (
    MaskingGenerator,
    SamplerType,
...
from rose.logging import MetricLogger, setup_logging
from rose.train.cosine_lr_scheduler import CosineScheduler, linear_warmup_cosine_decay
from rose.train.multidist_meta_arch import MultiDistillationMetaArch
from rose.train.ssl_meta_arch import SSLMetaArch
...
def get_args_parser(add_help: bool = True):
    parser = argparse.ArgumentParser("ROSE training", add_help=add_help)
    parser.add_argument("--config-file", default="rose/configs/train/vitl_derm_pretrain.yaml", metavar="FILE", help="path to config file")
    parser.add_argument(
        "--no-resume",
        action="store_true",
...
def build_data_loader_from_cfg(
    ...
    collate_fn = partial(
        collate_data_and_cast, # collate only data in current rank
