...
from rose.checkpointer import init_fsdp_model_from_checkpoint
from rose.configs import get_default_config
from rose.data import DataAugmentationDerm
from rose.fsdp.ac_compile_parallelize import ac_compile_parallelize
from rose.layers.dino_head import DINOHead
from rose.loss import DINOLoss, GramLoss, KoLeoLoss, KoLeoLossDistributed, iBOTPatchLoss, PCRLoss
from rose.models import build_model_from_cfg
from rose.train.cosine_lr_scheduler import linear_warmup_cosine_decay
from rose.train.param_groups import fuse_params_groups, get_params_groups_with_decay_fsdp
from rose.utils import count_parameters
...
class SSLMetaArch(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # PCR
        self.pcr_use_loss = self.cfg.pcr.use_loss
        self.pcr_bin_matrix = False
        if self.pcr_use_loss:
            # TODO: init pcr configuration
            self.pcr_loss = PCRLoss()
            self.pcr_loss_weight = self.cfg.pcr.loss_weight
            self.pcr_bin_matrix = self.cfg.pcr.pcr_bin_matrix
            self.ite_use_pcr = self.cfg.pcr.ite_use_pcr
            logger.info("OPTIONS -- PCR")
            logger.info(f"OPTIONS -- PCR -- loss_weight: {cfg.pcr.loss_weight}")
            logger.info(f"OPTIONS -- PCR -- pcr_bin_matrix: {cfg.pcr.pcr_bin_matrix}")
            logger.info(f"OPTIONS -- PCR -- ite_use_pcr: {cfg.pcr.ite_use_pcr}")
    ...
    def forward_backward(
        ...
        global_crops = data["collated_global_crops"].cuda(non_blocking=True) #[2*B, 3, 224, 224]
        ...
        if self.pcr_bin_matrix:
            pcr_bin_matrix = data["collated_sim_bin_matrices"].cuda(non_blocking=True)
        else:
            pcr_bin_matrix = None
        if self.has_gram_teacher:
    ...
    def compute_losses(
        ...
        *,
        ...
        # Pcr loss
        if self.pcr_use_loss and (iteration + 1) >= self.ite_use_pcr:
            student_patch = student_global["patch_pre_head"].flatten(0, 1)
            teacher_patch = teacher_global["patch_pre_head"].flatten(0, 1)
            unmasked_idx = (~masks.any(dim=1)).nonzero(as_tuple=True)[0]
            pcr_student_patch = student_patch[unmasked_idx]
            pcr_teacher_patch = teacher_patch[unmasked_idx]
            pcr_unmasked_matrix = pcr_bin_matrix[unmasked_idx]
            pcr_loss = self.pcr_loss(
                pcr_student_patch,
                pcr_teacher_patch,
                pcr_unmasked_matrix,
            )
            loss_dict["pcr_loss"]=pcr_loss
            loss_accumulator += self.pcr_loss_weight * pcr_loss
    ...
    def build_data_augmentation_dino(self, cfg):
        return DataAugmentationDerm(
            cfg.crops.global_crops_scale,
            cfg.crops.local_crops_scale,
            cfg.crops.local_crops_number,
            global_crops_size=cfg.crops.global_crops_size,
            local_crops_size=cfg.crops.local_crops_size,
            pcr_bin_matrix=cfg.pcr.pcr_bin_matrix,
