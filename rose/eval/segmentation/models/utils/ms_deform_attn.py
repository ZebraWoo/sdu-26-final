...
def ms_deform_attn_core_pytorch(value, value_spatial_shapes, sampling_locations, attention_weights):
    """Multi-scale deformable attention via F.grid_sample (pure PyTorch, CUDA kernels from PyTorch)."""
...
class MSDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4, ratio=1.0):
        ...
        _d_per_head = int(d_model * ratio) // n_heads
        if not _is_power_of_2(_d_per_head):
            warnings.warn(
                "d_model * ratio / n_heads is not a power of 2; some GPU kernels may be suboptimal."
            )
