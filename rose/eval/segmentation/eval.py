...
from rose.eval.segmentation.metrics import (
...
from rose.hub.segmentors import dinov3_vit7b16_ms
from rose.logging import MetricLogger
...
def evaluate_segmentation_model(
    ...
    if mode == "train":
        ...
        header = "Test: "
    for batch_img, (_, gt) in metric_logger.log_every(test_dataloader, 10, header=header):
...
def test_segmentation(backbone, config, mode):
    ...
    if mode == "train":
        ...
        test_dataset = DatasetWithEnumeratedTargets(
            make_dataset(
                dataset_str=f"{config.datasets.test}:root={config.datasets.root}",
                transforms=transforms,
            )
        )
    else:
        raise ValueError(f"Unsupported mode: {mode}, only 'train' and 'test' are supported!")
