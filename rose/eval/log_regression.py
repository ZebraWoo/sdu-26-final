...
from rose.data.transforms import CROP_DEFAULT_SIZE, get_target_transform, make_classification_eval_transform
from rose.distributed import get_rank, get_world_size
from rose.eval.data import (
...
from rose.eval.utils import average_metrics, evaluate, extract_features
from rose.eval.utils import save_results as default_save_results_func
from rose.run.init import job_context
from rose.utils.dtype import as_torch_dtype
...
class TrainConfig:
    ...
    batch_size: int = 128  # batch size for train and val set feature extraction
...
def eval_log_regression_with_model(*, model: torch.nn.Module, autocast_dtype, config: LogregEvalConfig):
    ...
    # Setting up metrics
    val_metric = build_classification_metric(config.train.val_metric_type, num_classes=num_classes, dataset=val_dataset, ks=(1,))
    test_metric_type = config.eval.test_metric_type or config.train.val_metric_type
    test_metric = build_classification_metric(test_metric_type, num_classes=num_classes, dataset=test_dataset, ks=(1,))
