...
from .extended_augmentations import RandomGaussianResizedCrop
from rose.data.transforms import SSL_DEFAULT_MEAN, SSL_DEFAULT_STD, GaussianBlur, make_normalize_transform
...
class DataAugmentationDerm(object):
    def __init__(
        self,
        global_crops_scale,
        local_crops_scale,
        ...
        self.pcr_bin_matrix = pcr_bin_matrix
        ...
        logger.info(f"pcr_bin_matrix: {pcr_bin_matrix}")
        ...
        self.geometric_augmentation_global = transforms.Compose(
            [
                ...
                #),
                RandomGaussianResizedCrop(
                   size=global_crop_max_size,
                    scale=global_crops_scale,
                    interpolation=transforms.InterpolationMode.BICUBIC,
                    mean=(0.5, 0.5),
                    std=(0.3, 0.3)
    ...
    def __call__(self, image):
        ...
        if self.pcr_bin_matrix:
            im1_base_t = transforms.ToTensor()(im1_base)
            im2_base_t = transforms.ToTensor()(im2_base)

            def img_to_patches(x, patch_size=16):
                C, H, W = x.shape
                x = x.unfold(1, patch_size, patch_size).unfold(2, patch_size, patch_size)
                x = x.contiguous().view(C, -1, patch_size, patch_size)  # [C, P_N, p_h, p_w]
                return x

            img1_patches = img_to_patches(im1_base_t, self.patch_size)
            img2_patches = img_to_patches(im2_base_t, self.patch_size)

            _, P_N, _, _ = img1_patches.shape

            # flatten
            img1_flat = img1_patches.permute(1, 0, 2, 3).reshape(P_N, -1)  # [P_N, C*p*p]
            img2_flat = img2_patches.permute(1, 0, 2, 3).reshape(P_N, -1)

            dist_matrix_11 = torch.cdist(img1_flat, img1_flat, p=2)  # [P_N, P_N]
            dist_matrix_22 = torch.cdist(img2_flat, img2_flat, p=2)

            sim_matrix_11 = torch.exp(-0.5 * dist_matrix_11)
            sim_matrix_22 = torch.exp(-0.5 * dist_matrix_22)

            sim_bin_11 = (sim_matrix_11 > 0.3).float()
            sim_bin_22 = (sim_matrix_22 > 0.3).float()

            output["pcr_bin_matrix"] = [sim_bin_11, sim_bin_22] # 2, [P_N, P_N]
