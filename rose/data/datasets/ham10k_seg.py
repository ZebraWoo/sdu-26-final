...
from .isic2018 import _Split, _load_segmentation


def _load_file_paths(root: str, split: _Split) -> Tuple[List[str], List[str]]:
    with open(os.path.join(root, f"HAM10000{split.value}.csv")) as f:
        df = pd.read_csv(f)
...
class ham10k_seg(ExtendedVisionDataset):
    Split = Union[_Split]
    Labels = Union[Image.Image]

    def __init__(
        self,
        split: "ham10k_seg.Split",
