...
def collate_data_and_cast(
    ...
    n_global_crops = len(samples_list[0][0]["global_crops"]) # [({...}, (label=none)),...]
    n_local_crops = len(samples_list[0][0]["local_crops"])

    collated_global_crops = torch.stack(
        [s[0]["global_crops"][i] for i in range(n_global_crops) for s in samples_list]
    )  # [n_global_crops*B,3, 224, 224]
    collated_local_crops = torch.stack(
        [s[0]["local_crops"][i] for i in range(n_local_crops) for s in samples_list])
    if "pcr_bin_matrix" in samples_list[0][0]:
        collated_sim_bin_matrices = torch.stack(
            [s[0]["pcr_bin_matrix"][i] for i in range(n_global_crops) for s in samples_list])  # [2B,n,n]

    else:
        collated_sim_bin_matrices = None
    ...
    if collated_sim_bin_matrices is not None:
        out["collated_sim_bin_matrices"] = collated_sim_bin_matrices
